...
class QuotePerformanceTest {
  ...
  async measureQuoteLatency(sellToken: string, buyToken: string, sellAmount: string): Promise<number> {
    const startTime = performance.now();
    
    try {
      const response = await axios.post(`${this.baseUrl}/api/unified-quote-simple`, {
        sellToken,
        buyToken,
        sellAmount,
        chainId: 1
  ...
  calculateMetrics(latencies: number[]): PerformanceMetrics {
  ...
  async testCommonPairs(iterations: number = 100): Promise<QuoteTestResult[]> {
    ...
    for (const pair of this.commonPairs) {
      ...
      for (let i = 0; i < 3; i++) {
        try {
          await this.measureQuoteLatency(
            pair.sellToken,
            pair.buyToken,
      ...
      for (let i = 0; i < iterations; i++) {
        try {
          const latency = await this.measureQuoteLatency(
            pair.sellToken,
            pair.buyToken,
            '1000000000000000000'
          );
  ...
  async testConcurrentLoad(concurrentUsers: number = 100, duration: number = 30000): Promise<{
      ...
      while (Date.now() < endTime) {
        ...
        try {
          ...
          const latency = await this.measureQuoteLatency(
            pair.sellToken,
            pair.buyToken,
            Math.floor(Math.random() * 10 + 1) + '000000000000000000' // Random 1-10 ETH
          );
          successfulRequests++;
          latencies.push(latency);
        } catch (error) {
...
export { QuotePerformanceTest, PerformanceMetrics, QuoteTestResult };

...
const routes = {
  ...
  'POST /api/quote-profitable': async (req, res) => {
    ...
    // Price in USD for each token (simulated market prices)
    ...
    const sellDecimals = tokenDecimals[sellTokenLower] || 18;
    const buyDecimals = tokenDecimals[buyTokenLower] || 18;
    
    // Get prices with dynamic pricing for unknown tokens
    let sellPriceUSD = tokenPricesUSD[sellTokenLower];
    let buyPriceUSD = tokenPricesUSD[buyTokenLower];
    
    // If token price not found, generate a consistent pseudo-random price based on address
    if (!sellPriceUSD) {
      // Use last 4 chars of address to generate a price between $0.01 and $100
      const hashValue = parseInt(sellTokenLower.slice(-4), 16);
      sellPriceUSD = 0.01 + (hashValue % 10000) / 100;
    }
    if (!buyPriceUSD) {
      const hashValue = parseInt(buyTokenLower.slice(-4), 16);
      buyPriceUSD = 0.01 + (hashValue % 10000) / 100;
    }
    
    // Calculate exchange rate
    const baseRate = sellPriceUSD / buyPriceUSD;
    const profitMargin = 0.003; // 0.3% profit margin
    const rate = baseRate * (1 - profitMargin);
    
    // Convert sell amount from base units to token units
    const sellAmountInTokens = parseFloat(sellAmount) / Math.pow(10, sellDecimals);
    
    // Calculate buy amount in token units
    const buyAmountInTokens = sellAmountInTokens * rate;
    
    // Convert buy amount to base units
    const buyAmountInBaseUnits = Math.floor(buyAmountInTokens * Math.pow(10, buyDecimals)).toString();

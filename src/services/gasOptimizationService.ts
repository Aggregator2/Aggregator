...
interface RouteComparison {
  ...
  savings: {
    gasSavingsWei: bigint;
    gasSavingsUSD: number;
    gasSavingsPercent: number;
    outputImprovement: number;
    outputImprovementPercent: number;
  };
}
...
class GasOptimizationService {
  ...
  private gasHistoryCache: Map<string, GasEstimate[]> = new Map();
  private readonly ETH_PRICE_URL = 'https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd';
  private ethPriceUSD = 2000; // Default fallback
  ...
  private async updateETHPrice() {
    try {
      const response = await axios.get(this.ETH_PRICE_URL);
  ...
  async estimateGasForRoute(
    ...
    try {
      ...
      return {
        ...
        gasPrice,
        totalCostWei,
        totalCostUSD: Number(ethers.formatEther(totalCostWei)) * this.ethPriceUSD,
  ...
  async compareRoutes(
    ...
    directDEXCalldatas: { protocol: string; calldata: string }[],
    routerAddress: string,
    userAddress: string,
    inputAmount: string,
    expectedOutputs: { aggregator: string; dexes: { protocol: string; output: string }[] }
  ): Promise<RouteComparison> {
    // Estimate gas for aggregator route
    const aggregatorGas = await this.estimateGasForRoute(
      aggregatorCalldata,
      routerAddress,
      userAddress,
      inputAmount
    ...
    const dexEstimates = await Promise.all(
      directDEXCalldatas.map(async (dex) => {
        const gasEstimate = await this.estimateGasForRoute(
          dex.calldata,
          routerAddress,
          userAddress,
          inputAmount
    ...
    const mostEfficientDEX = dexEstimates.reduce((best, current) => 
      current.gasEstimate.totalCostWei < best.gasEstimate.totalCostWei ? current : best
    );
    ...
    return {
      ...
      savings: {
        gasSavingsWei,
        gasSavingsUSD: Number(ethers.formatEther(gasSavingsWei)) * this.ethPriceUSD,
        gasSavingsPercent,
        outputImprovement: Number(ethers.formatUnits(outputImprovement, 18)),
  ...
  private countRouteSteps(calldata: string): number {
  ...
  private addToGasHistory(contract: string, estimate: GasEstimate) {
  ...
  getGasHistory(contract: string): GasEstimate[] {
    return this.gasHistoryCache.get(contract) || [];
  }

  getAverageGasForContract(contract: string): GasEstimate | null {
  ...
  async optimizeRoute(
    ...
    gasComparison: Map<string, GasEstimate>;
  }> {
    const gasComparison = new Map<string, GasEstimate>();
    
    // Estimate gas for each route
    const routesWithGas = await Promise.all(
  ...
  async getGasPriceStats(): Promise<{
    slow: bigint;
    standard: bigint;
    fast: bigint;
    instant: bigint;
